...
// A checkup is generated by a background worker, not by the request that asks
// for it: on a reasoning model it runs for minutes, and a phone that locks its
// screen used to throw away the whole report. The client queues a job and then
// follows it, or simply comes back to a push notification.
export interface CheckupJobAccepted {
  job_id: string
  status: CheckupJobStatus
  period: CheckupPeriod
  period_label: string
  message: string
}

export type CheckupJobStatus = 'queued' | 'processing' | 'succeeded' | 'failed'

export interface CheckupJob {
  id: string
  status: CheckupJobStatus
  period: CheckupPeriod
  period_label: string
...
export function isCheckupJobRunning(job: Pick<CheckupJob, 'status'>) {
  return job.status === 'queued' || job.status === 'processing'
}

export async function enqueueCheckup(period: CheckupPeriod): Promise<CheckupJobAccepted> {
  ...
  if (!res.ok) {
    throw new Error(formatChatError(res.status, await res.text()))
  }
  return (await res.json()) as CheckupJobAccepted
}

export async function getCheckupJob(id: string): Promise<CheckupJob> {
  const res = await fetch(`/api/v1/ai/checkup/jobs/${encodeURIComponent(id)}`)
  if (!res.ok) {
    throw new Error(formatChatError(res.status, await res.text()))
  }
  return (await res.json()) as CheckupJob
}

export async function listCheckupJobs(): Promise<CheckupJob[]> {
  const res = await fetch('/api/v1/ai/checkup/jobs')
  if (!res.ok) {
    throw new Error(formatChatError(res.status, await res.text()))
  }
  return (await res.json()) as CheckupJob[]
