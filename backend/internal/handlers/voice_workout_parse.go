...
func buildVoiceParsePrompt(candidates []voiceExerciseCandidate, foods []voiceFoodCandidate, projects []taskProject, draft []voiceParsedExercise, workoutOpen bool, now time.Time) string {
	...
	sb.WriteString("- Вместе с cooked заполни cook_form - что это за еда: meat (мясо, птица), fish (рыба, морепродукты), pasta (макароны), rice (рис), buckwheat (гречка), legume (бобовые, чечевица), vegetable (овощи). Не подходит ничего - оставь пустым.\n")
