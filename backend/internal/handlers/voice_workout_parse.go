...
func buildVoiceParsePrompt(candidates []voiceExerciseCandidate, foods []voiceFoodCandidate, projects []taskProject, draft []voiceParsedExercise, workoutOpen bool, now time.Time) string {
	...
	sb.WriteString(`{"domain":"food","exercises":[],"entries":[{"food_id":"...","serving_id":"...","name":"...","grams":70,"meal":"breakfast","cooked":false,"cook_form":""}],"unmatched":["..."]}`)
	...
	sb.WriteString("- Общее название - это не \"нет в списке\". Люди говорят \"макароны\", \"курица\", \"творог\", а в списке лежит конкретный товар с брендом: \"Макфа Макароны Отварные\", \"Петелинка Куриное Филе\". Найди в списке то, что человек и правда ест под этим словом, и выбери самый частый подходящий вариант (rank меньше = чаще).\n")
	sb.WriteString("- Услышав готовую еду (\"варёные макароны\", \"жареная курица\"), выбирай сырой продукт от производителя. Любая запись в списке со словами варёное, вареные, отварные, жареное, запечённое - это чей-то ручной ввод, а у сырого КБЖУ с упаковки. Для \"варёных макарон\" это \"Barilla Макароны\", а не \"Макфа Вареные Макароны\" и не \"Макфа Макароны Отварные\"; для \"жареной курицы\" - \"Петелинка Куриное Филе\", а не \"Бахетле Бедро Куриное Жареное\".\n")
	sb.WriteString("- cooked ставь true всегда, когда еда названа готовой: варёная, жареная, отварная, запечённая, приготовленная, тушёная, на пару, с гриля. grams оставляй как сказано - это вес готового, backend сам пересчитает его в вес сырого.\n")
	sb.WriteString("- Вместе с cooked заполни cook_form - что это за еда: meat (мясо, птица), fish (рыба, морепродукты), pasta (макароны), rice (рис), buckwheat (гречка), legume (бобовые, чечевица), vegetable (овощи), egg (яйца). Не подходит ничего - оставь пустым.\n")
	sb.WriteString("- Готовый вариант из списка бери только если сырого там нет или это готовое блюдо, которое взвешивают как есть: шаурма, лазанья, сэндвич, котлета из кулинарии. Тогда cook_form оставь пустым.\n")
	sb.WriteString("- Названный без уточнений продукт - это он сам, а не блюдо с ним. \"Курица\" это \"Петелинка Куриное Филе\", а не \"ВкусВилл Курица с Картофельным Хэшбрауном в Тортилье\", даже если тортилью человек ест чаще. Блюдо бери только если его и назвали: шаурма, сэндвич, лапша с курицей.\n")
	sb.WriteString("- Часть тушки, бренд, жирность и помол человек вслух обычно не называет. Это не повод отказываться: \"курица\" - бери самый частый простой куриный товар, \"творог\" - самый частый творог. Выбрать конкретный товар под общее слово - это и есть твоя работа, а не \"замена продукта\".\n")
	sb.WriteString("- В unmatched фраза уходит только если в списке нет самого продукта - человек съел то, чего раньше никогда не ел. Съеденное однажды лучше пропустить, чем записать чужой продукт.\n")
