...
func buildVoiceParsePrompt(candidates []voiceExerciseCandidate, foods []voiceFoodCandidate, projects []taskProject, draft []voiceParsedExercise, workoutOpen bool, now time.Time) string {
	...
	sb.WriteString(`{"domain":"food","exercises":[],"entries":[{"food_id":"...","serving_id":"...","name":"...","grams":70,"meal":"breakfast","cooked":false}],"unmatched":["..."]}`)
	...
	sb.WriteString("- Способ приготовления - это часть продукта, а не уточнение. Услышав \"варёные макароны\" или \"жареную курицу\", ищи в списке вариант именно с этим словом (\"Макфа Макароны Отварные\", \"Бахетле Бедро Куриное Жареное\") и бери его, даже если сырой вариант встречается чаще. Человек называет вес готового, а сырой продукт весит другое: курица теряет воду, макароны её набирают.\n")
	sb.WriteString("- Если варианта с нужным способом приготовления в списке нет, возьми обычный, но поставь cooked: true - человека предупредят, что вес записан против сырого продукта.\n")
	sb.WriteString("- cooked ставь true всегда, когда еда названа готовой: варёная, жареная, отварная, запечённая, приготовленная, на пару, с гриля.\n")
