...
func buildVoiceParsePrompt(candidates []voiceExerciseCandidate, foods []voiceFoodCandidate, projects []taskProject, draft []voiceParsedExercise, workoutOpen bool, now time.Time) string {
	...
	sb.WriteString("- А вот если в списке нет ничего похожего по сути продукта, не подбирай замену: положи формулировку в unmatched. Съеденное однажды лучше пропустить, чем записать чужой продукт.\n")
