...
func buildVoiceParsePrompt(candidates []voiceExerciseCandidate, foods []voiceFoodCandidate, projects []taskProject, draft []voiceParsedExercise, workoutOpen bool, now time.Time) string {
	...
	sb.WriteString("- Способ приготовления не мешает выбрать продукт и никогда не повод отказаться. \"Варёные макароны\" это макароны, \"жареная курица\" это курица: выбирай товар по сути еды. Список уже отсортирован так, что сырой товар производителя стоит выше чужих записей о готовом - бери верхний подходящий.\n")
	sb.WriteString("- Когда еда названа готовой - варёная, жареная, отварная, запечённая, тушёная, на пару, с гриля - поставь cooked: true и cook_form: meat (мясо, птица), fish (рыба), pasta (макароны), rice (рис), buckwheat (гречка), legume (бобовые), vegetable (овощи), egg (яйца). Не подходит ничего - оставь cook_form пустым. grams оставляй как сказано: это вес готового, backend сам пересчитает его в вес сырого продукта.\n")
	...
	sb.WriteString("- units указывай только когда пользователь назвал именно порции или штуки. Не записывай граммы в units.\n")
	sb.WriteString("- Одна штука, пачка, батончик или пакетик - это usual_units этого продукта, а не единица. Поэтому считай так: units = количество × usual_units. \"2 пачки\" мармелада с usual_units 0.7 это units = 1.4, а не 2; \"полторы порции\" продукта с usual_units 1 это units = 1.5.\n")
