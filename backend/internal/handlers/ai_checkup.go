...
func (h *AIHandler) appendCheckupHealthContext(ctx context.Context, sb *strings.Builder, userID string, window checkupWindow) {
	...
	`, userID, window.Start, window.End).Scan(&totalSteps, &avgSteps, &stepDaysCount)
	...
	_ = h.db.QueryRow(ctx, `
		...
		LIMIT 1
	`, userID, window.Start, window.End).Scan(&bestStepDay, &bestSteps)
	...
	firstWeightErr := h.db.QueryRow(ctx, `
		...
		LIMIT 1
	`, userID, window.Start, window.End).Scan(&firstWeight.value, &firstWeight.timestamp)
	lastWeightErr := h.db.QueryRow(ctx, `
		...
		LIMIT 1
	`, userID, window.Start, window.End).Scan(&lastWeight.value, &lastWeight.timestamp)
	...
	h.db.QueryRow(ctx, `
		...
		WHERE user_id = $1
			...
			AND timestamp >= $2
			AND timestamp < $3
	`, userID, window.Start, window.End).Scan(&avgRestingHR, &restingCount)
	if restingCount > 0 {
	...
	h.db.QueryRow(ctx, `
		...
		WHERE user_id = $1
			AND date >= $2::date
			AND date <= $3::date
	`, userID, window.Start, window.End).Scan(&sleepSessions, &avgSleepMinutes)
	if sleepSessions > 0 {
