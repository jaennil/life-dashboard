...
func buildAICheckupPrompt(now time.Time, window checkupWindow, dataContext string) string {
	...
	sb.WriteString("Отдельно обрати внимание на tool daily_series: это сводный дневной ряд за 90 дней, где все сферы стоят на одних и тех же строках. Только он позволяет сопоставлять сферы между собой, остальные секции описывают каждую сферу по отдельности.\n")
	sb.WriteString("В daily_series пустая клетка означает отсутствие данных, а не ноль. Поле days_with_data показывает, по сколько дней реально есть значения в каждом столбце.\n")
	sb.WriteString("Не строй вывод о связи, если по любому из двух столбцов данных меньше 20 дней: вместо вывода напиши, что данных недостаточно, и назови столбец и число дней.\n")
	sb.WriteString("Любое число из daily_series называй вместе с его окном: \"за 90 дней\", \"в дни короткого сна за 90 дней\". Это шире периода отчёта, поэтому среднее оттуда не равно среднему за период и не должно подаваться как показатель периода. Если число из daily_series спорит с числом из секции периода - например, среднее шагов выше, чем лучший день недели, - значит оно про другое окно, и это надо написать прямо.\n")
	...
	sb.WriteString("10. Зависимости и гипотезы.\n")
	sb.WriteString("11. Три конкретных шага на следующий период.\n")
	...
	sb.WriteString("Для каждой гипотезы обязательно приведи числа, на которых она держится: сравни средние по группам дней и укажи, сколько дней в каждой группе. Без чисел гипотезу не выдвигай.\n")
	sb.WriteString("Формулируй как гипотезу, а не как факт, и не выдавай совпадение за причину. Если видишь третье объяснение — например будни против выходных — назови его.\n")
	sb.WriteString("Если связи не видно, честно напиши, что не нашёл, и не придумывай красивую.\n")
	sb.WriteString("Рекомендации в разделе 11 должны опираться на найденные зависимости и на конкретные числа, а не быть общими советами про больше спать и меньше тратить.\n")
	sb.WriteString("Не делай длинное эссе: разделы 1-9 держи компактными, объём тратить на разделы 10 и 11.\n\n")
