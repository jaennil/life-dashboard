...
func LoadDueSyncs(ctx context.Context, db *pgxpool.Pool, source string, now time.Time) ([]DueSync, error) {
	rows, err := db.Query(ctx, `
		SELECT ss.user_id, ss.last_synced_at, ss.last_failed_at, ss.consecutive_failures, u.last_active_at
		FROM sync_state ss
	...
	for rows.Next() {
		var item DueSync
		if err := rows.Scan(&item.UserID, &item.LastSyncedAt, &item.LastFailedAt, &item.ConsecutiveFailures, &item.LastActiveAt); err != nil {
			return nil, err
		}
		if IsDue(now, source, item.LastActiveAt, item.LastSyncedAt, item.LastFailedAt, item.ConsecutiveFailures) {
			due = append(due, item)
		}
	}
...
func sourceIntervalForActivity(source string, now time.Time, lastActiveAt *time.Time) (time.Duration, bool) {
	tier := activityTier(now, lastActiveAt)
